...
import { Product, productsData } from "../../data/allProductsData";
    ...
    reducers: {
        filterByCategory: (
            ...
            state.filteredProducts = filtered.filter(
                (item) =>
                    item.retailPrice >= state.minPrice &&
                    item.retailPrice <= state.maxPrice
            );
        },
        filterByBrands: (
            state,
            action: PayloadAction<{ brands: string[] }>
        ) => {
        ...
        filterBySearch: (
            ...
            let filtered = state.allProducts;
            filtered = filtered.filter((item) =>
                item.title.startsWith(action.payload.searchText)
            );
            filtered = filtered.filter((item) =>
                item.category.startsWith(action.payload.searchText)
            );
            filtered = filtered.filter((item) =>
                item.brand.startsWith(action.payload.searchText)
            );

...
interface FiltersProps {
    ...
    value: number[];
    setValue: (value: number[]) => void;
    currentPage: number; // Add currentPage as prop
    setCurrentPage: (page: number) => void; // Add setCurrentPage as prop
}
...
const Filters: React.FC<FiltersProps> = ({
    ...
    let categoriesData = categories;
    if (searchParams.get("q") && filteredProducts.length > 0) {
        // Show all categories from filteredProducts when searching
        let uniqueCategoryIds = new Set(
            filteredProducts.map((item) => item.category.id)
        );
        categoriesData = categories.filter((item) =>
    ...
    let brandsData = brands;
    if (searchParams.get("q") && filteredProducts.length > 0) {
        // Show all brands from filteredProducts when searching
        ...
        // Optionally filter brands by selectedCategory if set
        if (selectedCategory !== "") {
        ...
        // Filter brands by selectedCategory when no search
        brandsData = brands.filter((brand) =>
            brand.categories.some(
                (category) => category.name === selectedCategory
            )
        ...
        // Show all brands when no search and no category selected
        brandsData = brands;
    }
